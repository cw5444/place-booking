...
import { useSupabase } from "../auth/provider"; // 프로바이더에서 가져옴
...
export default function AdminBookingsPage() {
  // 핵심: lib에서 직접 가져오지 않고 provider의 session과 supabase를 사용
  ...
  React.useEffect(() => {
    // 인증 로딩이 끝났는데 세션이 없으면 로그인 안 된 것
    if (!authLoading) {
      if (!session) {
        // 구글 로그인이 안 되어 있다면 여기서 체크가 됨
        alert("관리자 권한이 필요합니다.");
        router.push("/"); // 혹은 홈으로
        return;
      }
  ...
  if (authLoading || loading) return <div style={{ padding: 20 }}>인증 및 데이터 로드 중...</div>;
    ...
    <main style={{ padding: 20, maxWidth: 1000, margin: "0 auto", fontFamily: "sans-serif" }}>
      ...
      <div style={{ 
        ...
        <div>
          ...
          <p style={{ margin: '4px 0 0', color: '#b91c1c', fontSize: '13px' }}> 점검이나 내부 행사 시 해당 페이지에서 슬롯을 막을 수 있습니다.</p>
        </div>
        <button 
